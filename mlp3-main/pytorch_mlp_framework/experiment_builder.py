...
class ExperimentBuilder(nn.Module):
    def __init__(self, network_model, experiment_name, num_epochs, train_data, val_data,
        ...
        #Takes in learning rate
        learning_rate = float(experiment_name[experiment_name.index("&")+1:])
    ...
    def plot_grad_flow(self, named_parameters):
        ...
        """
        ########################################
        for name,item in named_parameters:
            if not "bias" in name:
                layers.append(name.replace('dict','').replace('.',' ').replace('_',' '))
                all_grads.append(torch.mean(torch.abs(item.grad)))
