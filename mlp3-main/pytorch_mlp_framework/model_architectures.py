...
class ConvolutionalProcessingBlockSpecial(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(ConvolutionalProcessingBlockSpecial, self).__init__()
    ...
    def forward(self, x):
        meanReg = x - torch.mean(x)
        stdReg = meanReg / torch.std(meanReg)
        out = (stdReg)
...
class ConvolutionalDimensionalityReductionBlockSpecial(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor):
        super(ConvolutionalDimensionalityReductionBlockSpecial, self).__init__()
